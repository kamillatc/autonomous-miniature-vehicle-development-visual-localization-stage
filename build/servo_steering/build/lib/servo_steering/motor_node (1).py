...
Tópicos publicados:
  /motor/pwm_duty   (std_msgs/Float32) → duty cycle aplicado: -1.0 a +1.0
                                          negativo = ré, positivo = frente
"""
...
class MotorNode(Node):
    def __init__(self):
        ...
        # Publisher de PWM — sinalizado: positivo=frente, negativo=ré
        self._pub_pwm = self.create_publisher(Float32, "/motor/pwm_duty", 10)
    ...
    def _publicar_pwm(self, duty_com_sinal: float):
        """Publica duty cycle com sinal em /motor/pwm_duty."""
        self._duty_atual = duty_com_sinal
        msg = Float32()
        msg.data = float(duty_com_sinal)
    ...
    def _acionar(self, velocidade: float):
        ...
        if velocidade > 0:
            ...
            self.get_logger().info(
                f"FRENTE {velocidade:.1f}% → duty {potencia:.3f} ({potencia*100:.1f}%)")
            ...
            self.get_logger().info(
                f"TRÁS   {abs(velocidade):.1f}% → duty {potencia:.3f} ({potencia*100:.1f}%)")
