...
class MotorNode(Node):
    def __init__(self):
        ...
        # DEBUG: publica o duty cycle PWM REAL aplicado no GPIO (já com o
        # limitador de tensão DUTY_MAX aplicado), pra acompanhar com:
        #   ros2 topic echo /debug/motor_pwm
        self._debug_pwm_pub = self.create_publisher(Float32, '/debug/motor_pwm', 10)
    ...
    def _acionar(self, velocidade: float):
        ...
        # DEBUG: publica o duty cycle REAL (0.0 a 1.0), com sinal indicando
        # direção (+ = frente, - = trás) — esse é o valor que realmente sai
        # no pino, já depois do limitador de tensão DUTY_MAX
        pwm_real_com_sinal = potencia if velocidade >= 0 else -potencia
        self._debug_pwm_pub.publish(Float32(data=float(pwm_real_com_sinal)))

        if velocidade > 0:
            self._lpwm.value = 0
            self._rpwm.value = potencia
            self.get_logger().info(
                f"linear.x → FRENTE  pedido={abs(velocidade):.1f}%  "
                f"PWM real={potencia*100:.1f}% (duty_max={DUTY_MAX*100:.1f}%)"
            )
            ...
            self.get_logger().info(
                f"linear.x → TRÁS    pedido={abs(velocidade):.1f}%  "
                f"PWM real={potencia*100:.1f}% (duty_max={DUTY_MAX*100:.1f}%)"
            )
    ...
    def _parar(self):
        ...
        self._debug_pwm_pub.publish(Float32(data=0.0))
